...
import { ApiTags, ApiOperation, ApiBody, ApiQuery, ApiResponse, ApiSecurity, ApiBearerAuth } from '@nestjs/swagger';
...
export class InvoiceController {
  ...
  create(@Body() data: CreateInvoiceDto, @Req() req: Request) {
  ...
  update(@Param('id') id: string, @Body() data: UpdateInvoiceDto, @Req() req: Request) {
    const token = (req.headers['authorization'] as string)?.replace('Bearer ', '') ?? '';
